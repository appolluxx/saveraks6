...
// Enhanced Hamming Distance with error handling
...
// Enhanced histogram similarity using correlation
function histogramCorrelation(hist1: number[], hist2: number[]): number {
...
// Extract basic EXIF-like metadata from image
async function extractImageMetadata(imageBuffer: Buffer): Promise<ExifData> {
    try {
        const metadata = await sharp(imageBuffer).metadata();
        return {
            imageWidth: metadata.width,
            imageHeight: metadata.height,
            software: 'web-app', // Web uploads don't preserve camera EXIF
        };
    } catch (error) {
        return {};
    }
...
export function generateDeviceFingerprint(req: any): string {
    const fingerprint = [
        ...
        req.ip || '',
        req.get('Sec-CH-UA') || '', // Chrome user agent hints
...
export class EnhancedAntiCheatService {
    ...
    private static async computeAHash(imageBuffer: Buffer): Promise<string> {
        ...
        // Calculate average
        const sum = resized.reduce((acc, val) => acc + val, 0);
    ...
    private static async computeColorHistogram(imageBuffer: Buffer): Promise<number[]> {
        ...
        const histogram = Array.from(data).map(v => v / 255);
        return histogram;
    }
    ...
    static async checkDuplicate(
        ...
        const behaviorCheck = await this.analyzeUserBehavior(userId);
        if (behaviorCheck.flaggedSubmissions > behaviorCheck.totalSubmissions * 0.3) {
            return {
                isDuplicate: true,
                confidence: 0.8,
                detectionMethod: 'behavior',
                requiresReview: true,
                reason: 'High flag rate detected'
            };
        }
    ...
    private static async calculateSimilarity(
        ...
        // Hash comparisons
        const pHashDist = hammingDistance(fingerprint.pHash, candidate.pHash);
        ...
        const histCorrelation = candidate.histogram ? 
            histogramCorrelation(fingerprint.histogram, candidate.histogram) : 0;
    ...
    /**
     * Analyze user submission behavior
     */
    private static async analyzeUserBehavior(userId: string): Promise<UserBehavior> {
        const now = new Date();
        const hourAgo = new Date(now.getTime() - 60 * 60 * 1000);
        const dayAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);

        const [recentCount, hourlyCount, flaggedCount, totalCount, lastSubmission] = await Promise.all([
            prisma.ecoAction.count({ where: { userId, createdAt: { gt: dayAgo } } }),
            prisma.ecoAction.count({ where: { userId, createdAt: { gt: hourAgo } } }),
        ...
        return {
            submissionFrequency: hourlyCount,
            averageSimilarity: 0, // Would need historical similarity data
